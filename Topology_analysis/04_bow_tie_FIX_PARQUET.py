'''
Bow-tie decomposition with:
  - parquet null models (10-50x faster loading)
  - checkpointing (resume from last completed null model)
  - optimized bow-tie: single BFS per direction, replaces nx.descendants
  - G.reverse(copy=False) to avoid RAM doubling
  - psutil memory logging throughout
  - sequential null model processing (safe for 64GB RAM)
'''
...
NULL_MODELS_DIR    = INPUT_DATA_DIR / "null_models_parquet"   # parquet, not graphml
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_FILE, mode='a'),  # append so reruns preserve history
